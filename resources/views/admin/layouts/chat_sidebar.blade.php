<!-- Sidebar -->
<div class="sidebar" id="sidebar">
    <div class="sidebar-inner slimscroll">
        <div class="sidebar-menu">
            <ul>
                <li>
                    <a href="admin-dashboard.php"><i class="la la-home"></i> <span>Back to Home</span></a>
                </li>
                <li class="menu-title"><span>Chat Groups</span> <a href="#" data-bs-toggle="modal" data-bs-target="#add_group"><i class="fa fa-plus"></i></a></li>
                <li>
                    <a href="chat.php">
                        <span class="chat-avatar-sm user-img">
                            <img class="rounded-circle" alt="" src="{{asset('public/assets/img/user.jpg')}}">
                        </span>
                        <span class="chat-user">#General</span>
                    </a>
                </li>
                <li>
                    <a href="chat.php">
                        <span class="chat-avatar-sm user-img">
                            <img class="rounded-circle" alt="" src="{{asset('public/assets/img/user.jpg')}}">
                        </span>
                        <span class="chat-user">#Video Responsive Survey</span>
                    </a>
                </li>
                <li>
                    <a href="chat.php">
                        <span class="chat-avatar-sm user-img">
                            <img class="rounded-circle" alt="" src="{{asset('public/assets/img/user.jpg')}}">
                        </span>
                        <span class="chat-user">#500rs</span>
                    </a>
                </li>
                <li>
                    <a href="chat.php">
                        <span class="chat-avatar-sm user-img">
                            <img class="rounded-circle" alt="" src="{{asset('public/assets/img/user.jpg')}}">
                        </span>
                        <span class="chat-user">#warehouse</span>
                    </a>
                </li>
                <li class="menu-title">Direct Chats <a href="#" data-bs-toggle="modal" data-bs-target="#add_chat_user"><i class="fa fa-plus"></i></a></li>
                <li>
                    <a href="chat.php">
                        <span class="chat-avatar-sm user-img">
                            <img class="rounded-circle" alt="" src="{{asset('public/assets/img/profiles/avatar-02.jpg')}}"><span class="status online"></span>
                        </span>
                        <span class="chat-user">John Doe</span> <span class="badge rounded-pill bg-danger">1</span>
                    </a>
                </li>
                <li>
                    <a href="chat.php">
                        <span class="chat-avatar-sm user-img">
                            <img class="rounded-circle" alt="" src="{{asset('public/assets/img/profiles/avatar-09.jpg')}}"><span class="status offline"></span>
                        </span>
                        <span class="chat-user">Richard Miles</span> <span class="badge rounded-pill bg-danger">7</span>
                    </a>
                </li>
                <li>
                    <a href="chat.php">
                        <span class="chat-avatar-sm user-img">
                            <img class="rounded-circle" alt="" src="{{asset('public/assets/img/profiles/avatar-10.jpg')}}"><span class="status away"></span>
                        </span>
                        <span class="chat-user">John Smith</span>
                    </a>
                </li>
                <li class="active">
                    <a href="chat.php">
                        <span class="chat-avatar-sm user-img">
                            <img class="rounded-circle" alt="" src="{{asset('public/assets/img/profiles/avatar-05.jpg')}}"><span class="status online"></span>
                        </span>
                        <span class="chat-user">Mike Litorus</span> <span class="badge rounded-pill bg-danger">2</span>
                    </a>
                </li>
            </ul>
        </div>
    </div>
</div>
<!-- /Sidebar -->
